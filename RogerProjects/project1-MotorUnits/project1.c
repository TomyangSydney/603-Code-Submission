...
double Kp_eye = 1.0; // 1.0; // 0.25;
double Kd_eye = 0.0198; // over 0.1 ;// under 0.005;// critical 0.0198; // 0.0037;
double passive_Kd_eye = 0.001;

// ARMS 
double Kp_arm =  80;// 2.5;
double Kd_arm =  11.2; // 1.075;
double passive_Kd_arm = 1.0;

// BASE TRANSLATION
double Kp_base_trans = 45.0; //45.0;
double Kd_base_trans = 15.0; // 15.0; 
double passive_Kd_base_trans = 2.0;

// BASE ROTATION
double Kp_base_rot =  200.0;// 100.0; //100.0;//80.0;
double Kd_base_rot =  30.0; // 45.0; // 45.0; //45.0;
double passive_Kd_base_rot = 2.0;
/*************************************************************************/
...
double time;
{
  ...
  // add it to store error for both joints
  double q1_error = 0.0, q2_error = 0.0;
    ...
    for (j=0; j<NARM_JOINTS; ++j) {
      ...
      if (ACTUATE_ARMS) {
          ...
          if (i == RIGHT) {
            // printf("%lf %lf \n", time, theta_error);
            if (j == 0) {
              q1_error = theta_error;
            }else {
              q2_error = theta_error;
            }
          }
...
double time;
{
  ...
  if ((roger->base_position[X] - roger->base_setpoint[X]) > 0.0001 && (roger->base_position[Y] != roger->base_setpoint[Y]) > 0.0001) {
    roger->base_setpoint[THETA] = atan2(roger->base_setpoint[Y] - roger->base_position[Y], roger->base_setpoint[X] - roger->base_position[X]);
  }
  ...
  // printf("Theta error is: theta=%6.4lf\n", theta_error);

  if (ACTUATE_BASE) {
    // REPLACE THE FOLLOWING LINE WITH THE PD CONTROLLER FOR THE BASE
    // USING YOUR GAINS
    Mz = Kp_base_rot * theta_error + Kd_base_rot * theta_dot_error;
    // PDBase_translate(roger, time);
  }
  else {
...
void project1_enter_params()
{
  ...
  printf("Base Rotates: K=%6.4lf  B=%6.4lf\n", Kp_base_rot, Kd_base_rot);
  printf("Base Rotates: enter 'K B'\n"); fflush(stdout);
