...
//   MUST end with %d.txt
#define Q_TABLE_FILE "./q_tables/qtable_9.txt"
#define CONVG_ACT 2
// end RLToolKit parameters

double project_9_q_table[NSTATES][NACTIONS] = {0.0};
Observation obs;
/**************************************************************/

// declare necessary functions
// !!!! TODO !!!!! declare any functions you use/define here!!!
bool stereo_observation(); // this is bool in my implementation
int fwd_arm_kinematics(), inv_arm_kinematics(), inv_arm_kinematics_errors();
void construct_wTb(), matrix_mult(), copy_errors();
void submit_errors(), add_error_arrays(), print_errors();
void LoadLearnedPolicy();
void Q_Learning(), Run_Policy();
double drand(double min_val, double max_val);
void random_location(double, double*, double*);
int reset_ball_training(Robot* roger, double time, int state, int internal_state[], int counter, int previous_action, int episode_len);
// int reset_ball_training();
void LoadLearnedQTable();
void home_arms();
void project5_visualize();
int SearchTrack(), Chase(), Touch(), Track(), ChaseTouch();
int VFClosure(Robot * roger, double errors[NDOF], double time);
int TFClosure(Robot * roger, double errors[NDOF], double time);
double NothingReward(int state, int previous_state, int previous_action, int internal_state[NACTIONS]);
...
double drand(min_val, max_val)
double min_val;
double max_val;
{
	double num = ((double) rand())/((double)RAND_MAX);
	return min_val + (max_val - min_val)*num;
}
...
double time;
{
    ...
    double larm_errors[NDOF], rarm_errors[NDOF], botharm_errors[NDOF];
	int obs_result = 0; // 
	int i;
	...
	if ((fabs(left_arm_forces[0]) > 0 || fabs(left_arm_forces[1]) > 0)
		...
		phi_wt = 3 + 2*cos(estimated_theta[0] - estimated_theta[1]) + 2*cos(estimated_theta[0] - estimated_theta[2]) + 2*cos(estimated_theta[1] - estimated_theta[2]);
	...
	if (return_state != NO_REFERENCE) {
		...
		// add_error_arrays(larm_errors, rarm_errors, botharm_errors);
		// add_error_arrays(botharm_errors, errors, errors);
		...
		// TODO-ADD your search-track function here 
		SearchTrack(roger, st_errors, time);
...
double time;
{
	...
	double estimated_theta[3] = {0.0};

	double botharms_errors[NDOF] = {0.0};
	printf("VFClosure called. \n");
	// print_errors(errors);
	// first do no harm
	// for (i=0; i<NDOF; ++i) {
	// 	errors[i] = 0.0;
...
int internal_state[];           // state of action in RL problem
...
{
	double roger_x = 0.0, roger_y = 0.0;            // current x, y position of Rogers base
	...
	if ((internal_state[CONVG_ACT] == CONVERGED && previous_action ==
		...
		if (converged_counter > 500 || (counter % episode_len == 0 && counter != 0)) {
			...
			printf("random_location_finished. \n");
...
double time;
{
    ...
    double alpha= 0.1, gamma= 0.9;         // learning rate, reward discounting factor
    int reward_num = 1;                   // total number of rewards used (MUST BE AT LEAST ONE)
    double default_reward = 0.0;         // reward applied for every decision 
	int end_episode_reward = 1;     // reward given for successful episode termination
	int end_episode_penalty = -1;   // penalty for not ending the episode in success

	// actions[0] = SearchTrack; 
	actions[0] = Chase;
	actions[1] = Touch; 
	actions[2] = TFClosure; 
	...
	// int (*reset_func_ptr)(Robot* roger, double time, int state, int internal_state[], int previous_action, int counter, int episode_len) = reset_ball_training; 
	// Q_Learning(roger, time, alpha, gamma, actions, rewards, default_reward, NACTIONS, NSTATES, 
	// 	reward_num, project_9_q_table, max_time_per_episode, num_episodes, eps_min, eps_max,
    ...
    if (policy_loaded == 0) {
		LoadLearnedQTable(q_table_num, project_9_q_table, NSTATES, NACTIONS, Q_TABLE_FILE);
		policy_loaded = 1;
	}

	// NOTE, DOES NOT construct empty folder inside of perf_data. If you want to put data in a sub-folder
	//     create the sub-folder first!
	generatePerformanceData(roger, time, actions, rewards, default_reward, NACTIONS, NSTATES, reward_num, project_9_q_table,
	performance_data_file_path, action_calls, num_trails, CONVG_ACT, end_episode_penalty, end_episode_reward,  max_time_per_episode, reset_ball_training); 
	/************************************************************/
}
