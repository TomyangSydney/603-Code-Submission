...
double x, y;
{
  ...
  if (-1.0 <= c2 && c2 <= 1.0) {
    if (limb == LEFT) {
      ...
      // set the reference point of left arm
      roger->arm_setpoint[LEFT][0] = theta1_minus; // error[4] = theta1_minus - roger->arm_theta[limb][0]
...
double errors[NDOF];
{
  ...
  roger->base_setpoint[0] = roger->base_position[X] + errors[0]*cos(roger->base_position[THETA] + errors[1]);
  roger->base_setpoint[1] = roger->base_position[Y] + errors[0]*sin(roger->base_position[THETA] + errors[1]);
...
double time;
{
  ...
  // get errors for LEFT shoulder (0) and elbow (1)
	errors[4] = 3.05 - roger->arm_theta[0][0];
	errors[5] = (-2.8) - roger->arm_theta[0][1];

  // get errors for RIGHT shoulder (0) and elbow (1)
